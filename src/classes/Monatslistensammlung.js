...
class Monatslistensammlung {
    ...
    _eintrag_hinzufuegen(eintrag){
        ...
        this._monatslisten.forEach(monatsliste => {
            if (monat === monatsliste.monat() && jahr === monatsliste.jahr()){
    ...
    _html_generieren(){
        ...
        section.setAttribute("id", "monatslisten");
        
        this._monatslisten.forEach(e => {
            section.insertAdjacentElement("afterbegin", e.html());
        });
